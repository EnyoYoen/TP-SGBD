...
select COUNT(case when lake_count = 0 then 1 end) as "Nombre de pays ayant 0 lac",
  ...
  from ( -- selectionne le code de chaque pays et son nombre de lacs
    select country.code, count(geo_lake.lake) as lake_count
      from country
      left join geo_lake -- au cas où il n'y a pas de lac dans le pays, il faut quand même renvoyer 0
